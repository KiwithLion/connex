<?php
    ...
    if (isset($_POST["submit-update"])){
      ...
      if(!filter_var($fname, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
          $msg_iv = "Please enter a valid name(characters only)!";
          $errorcount++;
      }

      // Validate surname
      if(!filter_var($sname, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
          $msg_iv = "Please enter a valid surname(characters only)!";
          $errorcount++;
      }
      
      // Validate email
      if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
          $msg_iv = "Please enter a valid email address!";
          $errorcount++;
      } 
      
      // Validate cell number
      if(!preg_match(("/^[0-9]{10}$/"), $number)){
          $msg_iv = "Please enter a 10 digit integer number!";
          $errorcount++;
      }
      ...
      if(!$uppC || !$lowC || !$numb || !$speC || strlen($pwd) < 8){
          $msg_iv = "Password should be atleast 8 chracters in length and should include atleast one upper, lowercase letter, one number, and one special character!";
          $errorcount++;
      }
      ...
      if ($errorcount === 0){
            ...
            $query = "UPDATE `admin_login` SET `admin_Fname`='$f_name',`admin_Email`='$email',`admin_Phone`='$number',`admin_Pwd`='$newPwd' WHERE admin_Type = '$user'";

            $result = mysqli_query($conn, $query);
            if ($result)
            {
                  $msg_v = "Admin your information is successfully updated!";
            }
            else
            {
                  $msg_iv = "Error while updating Admin information!";
            }
                  
      }
      else
      {
            $msg_iv .= "<br>ERROR number $errorcount";
      }
  }
?>
  ...
  <div id="_desktop_user_info">
    ...
    <section id="main">
      ...
      <section id="content" class="page-content">
            ...
            <?php
                  $select_user = mysqli_query($conn, "SELECT * FROM `admin_login` WHERE admin_Fname = '$user'") or die('query failed');
                  if(mysqli_num_rows($select_user) > 0){
                        while($fetch_product = mysqli_fetch_assoc($select_user)){
                              $fname = $fetch_product['admin_Fname'];
                              $email = $fetch_product['admin_Email'];
                              $phone = $fetch_product['admin_Phone'];
                              $pass = $fetch_product['admin_Pwd'];
                        }
                  };
            ?>
