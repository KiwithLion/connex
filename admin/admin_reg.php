<?php
    ...
    if (isset($_POST["submit-update"])){
      ...
      if (empty($fname) || empty($sname) || empty($email) || empty($number) || empty($pwd) || empty($cpwd)){
      ...
      if ($errorcount === 0){
            ...
            $check = mysqli_query($conn, "SELECT * FROM `admin_login` WHERE admin_Email = '$email'") or die('query failed');
						if(mysqli_num_rows($check) > 0){
							$msg_iv = "$fname already exist!";
						}else{
              //insert user into database
              $query = "INSERT INTO `admin_login`(admin_Fname, admin_Type, admin_Email, admin_Phone, admin_Pwd) VALUES('$f_name', '$type', '$email', '$number','$newPwd')";

              $result = mysqli_query($conn, $query);
              if ($result)
              {
                $msg_v = "Admin Registered successfully!";
              }
              else
              {
                $msg_iv = "Error while inserting values into database!";
              }
            }
                        ...
                        <form method="POST" action="">
                              <section class="form-fields">
                                    ...
                                    <div class="form-group row ">
                                          ...
                                          <div class="col-md-6">
                                                <input class="form-control" name="lastname" type="text" value="" maxlength="255" required="">
                                          </div>
                                          <div class="col-md-3 form-control-comment"></div>
                                    </div>
                                    <div class="form-group row ">
