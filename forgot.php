<?php
    ...
    if(isset($_POST['submit-getpass'])){
        ...
        if (empty($user))
        {
            $msgiv = "Email cannot be empty<br>";
        }
        //if(send_mail("Kiwithphetla@gmail.com", "Greetings", "How you doing my G?")){  
        $check = mysqli_query($conn, "SELECT * FROM `user_login` WHERE user_Email = '$user'") or die('query failed');
        if(mysqli_num_rows($check) > 0){
            ...
            if ($email === $user){
                $recipient = $user;
                $message = "$email you have requested a new password.<br> Copy this id and enter it to get your new password $token";
                /*$msg = "new password is $newPwd";
                if (send_mail($recipient, $subject, $message)){*/
            
                    /*insert user into database*/
                    $query = "INSERT INTO `getnewpass`(user_Email, user_Newpass) VALUES('$email', '$token')";
                    $result = mysqli_query($conn, $query);
                    if ($result)
                    {
                        //$msgv = "Email sent successfully!";
                        $msgv = "Enter this Token $token to enter your new password!";
                        header('refresh:5;newpass.php');
                    }
                    else
                    {
               ...
               /* }
                ...
                $msgiv = "Error, Emails don't match!";
            }
            
        }else{
            
                $msgiv = "$user doesn't exist!";
        }
}
