<?php
		...
		if(isset($_POST['submit-login'])){
			...
			$errorcount = 0; $count++;
		//$msg = "You in";
			// Vallidate All fields
			if (empty($name)){
				$msgiv = "Please enter your Name!";
				$errorcount++;
			} else if (empty($sname)){
						if ($errorcount < 1){
							$msgiv = "Please enter your Surname!";
							$errorcount++;
						}
				} else if (empty($email)){
							if ($errorcount < 1){
								...
								$errorcount++;
									...
									if ($errorcount < 1){
										$msgiv = "Please enter your Number!";
										$errorcount++;
									}
											} else if (empty($pwd)){
														if ($errorcount < 1){
															...
															$errorcount++;
																...
																if ($errorcount < 1){
																	$msgiv = "Please Confirm your Password!";
																	$errorcount++;
																}
															} 
			...
			if(!filter_var($name, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
				if ($errorcount < 1){
					$msgiv = "Please enter a valid name(characters only)!";
					$errorcount++;
				}
			}
	
			// Validate surname
			if(!filter_var($sname, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
				if ($errorcount < 1){
					$msgiv = "Please enter a valid surname(characters only)!";
					$errorcount++;
				}
			}
			...
			if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
				if ($errorcount < 1){
					$msgiv = "Please enter a valid email address!";
					$errorcount++;
				}
			} 
			...
			if(!preg_match(("/^[0-9]{10}$/"), $number)){
				if ($errorcount < 1){
					$msgiv = "Please enter a 10 digit integer number!";
					$errorcount++;
				}
			}
			
			// Validate password
			$uppC = preg_match('@[A-Z]@', $pwd);
			$lowC = preg_match('@[a-z]@', $pwd);
			$numb = preg_match('@[0-9]@', $pwd);
			$speC = preg_match('@[^\w]@', $pwd);
			if(!$uppC || !$lowC || !$numb || !$speC || strlen($pwd) < 8){
				if ($errorcount < 1){
					$msgiv = "Password should be atleast 8 chracters in length and should include atleast one upper, lowercase letter, one number, and one special character!";
					$errorcount++;
				}
			}
			...
			if($pwd !== $cpwd){
				if ($errorcount < 1){
					$msgiv = "Passwords don't match!";
					$errorcount++;
				}
			}
				
			//Check if fields aren't empty
			if (!empty($name) && !empty($sname) && !empty($email) && !empty($number) && !empty($pwd) && !empty($cpwd)){
				...
				if ($errorcount === 0){
					...
					//assign user type
					if (($email === "Admin@connex.com") || ($email === "admin@connex.com" ) || ($email === "admin@Connex.com"))
					{
						//send the user an email
						$subject = "Welcome Admin";
						$message = "<br> Congratulations Admin, you have successfully registered with connex.<br> You can view and edit the info.!";
						$recipient = $email;
						...
						$check = mysqli_query($conn, "SELECT * FROM `admin_login` WHERE admin_Email = '$email'") or die('query failed');
						if(mysqli_num_rows($check) > 0){
							$msgiv = "$fname already exist!";
						}else{

							//if (send_mail($recipient, $subject, $message)){
								//insert user into database
								$query = "INSERT INTO `admin_login`(admin_Fname, admin_Type, admin_Email, admin_Phone, admin_Pwd) VALUES('$fname', '$type', '$email', '$number','$newPwd')";
								$result = mysqli_query($conn, $query);
								if ($result)
								{
									$msgv = "Login successfully!";
					...
					$message = "<br>Congratulations $fname, you have registered with connex.<br> Enjoy our wide range of items!";
					$recipient = $email;

					//encryt Password
					$newPwd = md5($pwd);
					//check if user exists
					$check = mysqli_query($conn, "SELECT * FROM `user_login` WHERE user_Email = '$email'") or die('query failed');
					if(mysqli_num_rows($check) > 0){
						$msgiv = "$fname already exist!";
					}else{
						...
						//if (send_mail($recipient, $subject, $message)){
							//insert user into database
							$query = "INSERT INTO `user_login`(user_Fname, user_Email, user_Phone, user_Pwd) VALUES('$fname', '$email', '$number','$newPwd')";
							$result = mysqli_query($conn, $query);
							if ($result)
							{
								$msgv = "Registered successfully!";
								header('refresh:5;login.php');
							}
							else
							{
				...
				{
					$msgiv .= "<br>Error, error on error";
				}
			}
			else
			{
			$msgiv = "Error, Fields missing";
			}
		}
?>
<!DOCTYPE html>
...
clip:auto !important;
color-scheme:light !important;}#antdi6vf3a2g1683584364186.widget-hidden {display: none !important;}#antdi6vf3a2g1683584364186.widget-visible {display: block !important;}
@media print{
 #antdi6vf3a2g1683584364186.widget-visible { 
display: none !important;
 }
}</style><script src="https://cdn.jsdelivr.net/emojione/2.2.7/lib/js/emojione.min.js" type="text/javascript" async="" defer=""></script><script src="https://cdn.jsdelivr.net/emojione/2.2.7/lib/js/emojione.min.js" type="text/javascript" async="" defer=""></script><style type="text/css">@keyframes tawkMaxOpen{0%{opacity:0;transform:translate(0, 30px);;}to{opacity:1;transform:translate(0, 0px);}}@-moz-keyframes tawkMaxOpen{0%{opacity:0;transform:translate(0, 30px);;}to{opacity:1;transform:translate(0, 0px);}}@-webkit-keyframes tawkMaxOpen{0%{opacity:0;transform:translate(0, 30px);;}to{opacity:1;transform:translate(0, 0px);}}#hbu1r1ff167g1683584364254.open{animation : tawkMaxOpen .25s ease!important;}@keyframes tawkMaxClose{from{opacity: 1;transform:translate(0, 0px);;}to{opacity: 0;transform:translate(0, 30px);;}}@-moz-keyframes tawkMaxClose{from{opacity: 1;transform:translate(0, 0px);;}to{opacity: 0;transform:translate(0, 30px);;}}@-webkit-keyframes tawkMaxClose{from{opacity: 1;transform:translate(0, 0px);;}to{opacity: 0;transform:translate(0, 30px);;}}#hbu1r1ff167g1683584364254.closed{animation: tawkMaxClose .25s ease!important}</style></head>
...
<aside id="notifications">
  ...
  <section id="main">
			...
			<form action="" id="customer-form" class="js-customer-form" method="post">
				<section>
					...
					<div class="form-group row ">
						...
						<div class="col-md-6">
							...
							<span class="form-control-comment">
								Only letters and or followed by a space, are allowed.
							</span>
						</div>
					...
					<div class="form-group row ">
						...
						<div class="col-md-6">
							<input class="form-control" name="cell" type="number" value="">
						</div>
						<div class="col-md-3 form-control-comment"></div>
					</div>
					<div class="form-group row ">
					...
					<section>
						<footer class="form-footer text-sm-center clearfix">
							<button id="submit-login" name="submit-login" class="btn btn-primary" type="submit">
								Submit
							</button>
						</footer>
					</section>
	...
	<div class="fb-page fb_iframe_widget" data-href="https://web.facebook.com/profile.php?id=100090032495161" data-width="500" data-small-header="false" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true" fb-xfbml-state="rendered" fb-iframe-plugin-query="adapt_container_width=true&amp;app_id=&amp;container_width=292&amp;hide_cover=false&amp;href=https%3A%2F%2Fweb.facebook.com%2Fprofile.php%3Fid%3D100090032495161&amp;locale=en_US&amp;sdk=joey&amp;show_facepile=true&amp;small_header=false&amp;width=500"><span style="vertical-align: bottom; width: 292px; height: 130px;"><iframe name="f1c5fbe033092" width="500px" height="1000px" data-testid="fb:page Facebook Social Plugin" title="fb:page Facebook Social Plugin" frameborder="0" allowtransparency="true" allowfullscreen="true" scrolling="no" allow="encrypted-media" src="https://www.facebook.com/v2.6/plugins/page.php?adapt_container_width=true&amp;app_id=&amp;channel=https%3A%2F%2Fstaticxx.facebook.com%2Fx%2Fconnect%2Fxd_arbiter%2F%3Fversion%3D46%23cb%3Df212b7903e9404c%26domain%3Dconnexstore.co.za%26is_canvas%3Dfalse%26origin%3Dhttps%253A%252F%252Fconnexstore.co.za%252Ff398f66a3a795f8%26relation%3Dparent.parent&amp;container_width=292&amp;hide_cover=false&amp;href=https%3A%2F%2Fweb.facebook.com%2Fprofile.php%3Fid%3D100090032495161&amp;locale=en_US&amp;sdk=joey&amp;show_facepile=true&amp;small_header=false&amp;width=500" style="border: none; visibility: visible; width: 292px; height: 130px;" class=""></iframe></span></div>
</div>
...
<script type="text/javascript">
				...
				$(window).on("unload", function (e) {
					...
					pagetime.append("id_connections", "199107");
					pagetime.append("id_page", "65");
					pagetime.append("time_start", "2023-05-09 00:19:21");
					pagetime.append("token", "88ad6ce4fe62c595a3d814562f2e2b937fc2407e");
